...
public class MixedSlash : SkillFunction
{
    ...
    public override void StartSkill(BattleRoleData source, BattleRoleData target)
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            StartCoroutine(IEStartSkill(source, list[i]
                , valCaused(source,list[i],SkillKind.Physical)
                ,valCaused(source,list[i],SkillKind.Elemental)
                ,valCaused(source,list[i],SkillKind.Arcane))
                );
